...
exports.paramIdValidate = (req, text) => {
  ...
  const { value, meta } = joiValidation(req.params, schema, text);
  if (!_.isNil(meta)) {
    new Promise.reject(meta);
  }
  return value;
}

const joiValidation = (data, schema, method, allowUnknown = false) => {
  try {
    const { error, value } = schema.validate(data, { abortEarly: false, allowUnknown });
    if (!_.isNil(error)) {
...
exports.joiValidation = (data, schema, method, allowUnknown = false) => {
...
/**
 * Checking Authorization customer
 * @param {Object} req - req object
 * @param {Object} res - res object
 * @param {Object} next - next object
 ...
 */
exports.authorizedCustomer = async (req, res, next, err, user) => {
  console.log('i am called',err,user)
  if (err) {
    return next(err);
  }
  if (!user) {
  ...
  if (typeof user.access_token !== 'undefined' && user.access_token === tokenEncrypted) {
 ...
 */
exports.splitPhoneNumber = (data, key = 'phone_number') => {
  return new Promise((resolve, reject) => {
    try {
