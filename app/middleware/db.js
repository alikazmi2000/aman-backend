...
module.exports = {
   ...
   * @param {Object} select - selection string i.e. 'field1 field1'
   */
  async listInitOptions(req, select = '') {
   ...
   */
  async getItems(req, model, query, populate = false, project = '') {
    const options = await this.listInitOptions(req, project);
    if (populate !== false) {
   ...
   * @param {Object} project - projection keys
   */
  async getItemByQuery(query, model, rejectOnError = true, populate = false, project = {}) {
    return new Promise((resolve, reject) => {
      if (populate !== false) {
        model
          .findOne(query, project, (err, item) => {
        ...
        model.findOne(query, project, (err, item) => {
          if (rejectOnError === true) {
            itemNotFound(
              err,
              item,
  ...
  /** Get multiple records
     ...
     */
    async get(model, cond, proj = {}, limit = 0, skip = 0, sort = {},collation={}) {
  ...
  async getOne(id, model,proj={},populate = false) {
    return new Promise((resolve, reject) => {
      const message = localeMsg('ERROR.UNABLE_TO_FIND_ITEM', { name: model.modelName });
      if (populate !== false) {
        model
  ...
  async itemCompositeExists(cond, model, message) {
    return new Promise((resolve, reject) => {
      model.findOne(
        cond,
        (err, item) => {
