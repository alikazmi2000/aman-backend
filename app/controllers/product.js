	...
	try {
		if (req.query.search) {
			...
			if (req.query.category)
				Search = await db.get(Products, { 'name': { $regex: '.*' + req.query.search + '.*', $options: 'i' }, categories: req.query.category }, { name: 1 }, 10)

			else
				Search = await db.get(Products, { 'name': { $regex: '.*' + req.query.search + '.*', $options: 'i' } }, { name: 1 }, 10)
...
exports.getProductAvailability = async (req, res) => {
	try {
		...
		let result = [];
		result = await Promise.all(data.line_items.map(async item => {
			try {
		...
		let items = await Promise.all(data.line_items.map(async item => {
			try {
				...
				if (!_.isNull(result)) {
					...
					console.log(tax)
					let value = ProductServices.calculateTaxes(result, tax, item, data.address);
					return { ...item, value };
				}
				return { ...item, value: 0 };
				// console.log()
